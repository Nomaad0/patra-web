...
const DEMO_DATA={
  pea:[
    {id:"d1",name:"Amundi MSCI World CW8",ticker:"CW8.PA",quantity:80,pru:388,currentPrice:445,divPerShare:0,divFreq:"annuel"},
    {id:"d2",name:"Amundi PEA S&P 500 PE500",ticker:"PE500.PA",quantity:350,pru:44,currentPrice:52,divPerShare:0,divFreq:"annuel"},
    {id:"d3",name:"TotalEnergies",ticker:"TTE.PA",quantity:45,pru:50,currentPrice:56,divPerShare:3.2,divFreq:"trim"},
    {id:"d4",name:"Air Liquide",ticker:"AIR.PA",quantity:12,pru:160,currentPrice:178,divPerShare:3.5,divFreq:"annuel"},
    {id:"d11",name:"Schneider Electric",ticker:"SU.PA",quantity:10,pru:202,currentPrice:230,divPerShare:3.5,divFreq:"annuel"},
    {id:"d12",name:"LVMH",ticker:"MC.PA",quantity:4,pru:658,currentPrice:588,divPerShare:13.0,divFreq:"annuel"},
    {id:"d13",name:"BNP Paribas",ticker:"BNP.PA",quantity:35,pru:57,currentPrice:65,divPerShare:4.6,divFreq:"annuel"},
  ],
  cto:[
    {id:"d5",name:"Microsoft",ticker:"MSFT",quantity:12,pru:345,currentPrice:385,divPerShare:2.7,divFreq:"trim"},
    {id:"d14",name:"Apple",ticker:"AAPL",quantity:20,pru:170,currentPrice:189,divPerShare:1.0,divFreq:"trim"},
    {id:"d15",name:"Nvidia",ticker:"NVDA",quantity:40,pru:80,currentPrice:101,divPerShare:0.04,divFreq:"trim"},
    {id:"d16",name:"Amazon",ticker:"AMZN",quantity:12,pru:152,currentPrice:172,divPerShare:0,divFreq:"annuel"},
  ],
  crypto:[
    {id:"d6",name:"Bitcoin",symbol:"BTC",cgId:"bitcoin",quantity:0.18,avgPrice:65000,currentPrice:82000},
    {id:"d7",name:"Ethereum",symbol:"ETH",cgId:"ethereum",quantity:3.5,avgPrice:2100,currentPrice:1750},
    {id:"d17",name:"Solana",symbol:"SOL",cgId:"solana",quantity:20,avgPrice:98,currentPrice:125},
  ],
...
export default function PatrimoineTracker(){
  ...
  // Persistent storage
  useEffect(()=>{try{const raw=localStorage.getItem("patrimoine-v6");const isDemo=sessionStorage.getItem('patra-demo')||new URLSearchParams(window.location.search).get('demo')==='1';if(raw){const p=JSON.parse(raw);const hasPositions=!!(p.pea?.length||p.crypto?.length||p.cto?.length||p.livrets?.length);if(isDemo&&!hasPositions){sessionStorage.removeItem('patra-demo');window.history.replaceState(null,'','/app');const d=DEMO_DATA;setPea(d.pea);setCrypto(d.crypto);setCto(d.cto);setLivrets(d.livrets);setPeaCash(d.peaCash);setCtoCash(d.ctoCash);setCryptoCash(d.cryptoCash||0);setStablecoins(d.stablecoins||[]);setVersements(d.versements);setSnapshots(d.snapshots);setDivHistory(d.divHistory);setMonthlyIncome(d.monthlyIncome);if(d.targetAlloc)setTargetAlloc(d.targetAlloc);setTransactions(d.transactions);setIsDemo(true);}else{if(p.pea)setPea(p.pea);if(p.crypto)setCrypto(p.crypto);if(p.cto)setCto(p.cto);if(p.livrets)setLivrets(p.livrets);if(p.peaCash!==undefined)setPeaCash(p.peaCash);if(p.ctoCash!==undefined)setCtoCash(p.ctoCash);if(p.cryptoCash!==undefined)setCryptoCash(p.cryptoCash);if(p.stablecoins)setStablecoins(p.stablecoins);if(p.versements)setVersements(p.versements);if(p.snapshots)setSnapshots(p.snapshots);if(p.lastSync)setLastSync(p.lastSync);if(p.lastPeaSync)setLastPeaSync(p.lastPeaSync);if(p.lastCtoSync)setLastCtoSync(p.lastCtoSync);if(p.divHistory)setDivHistory(p.divHistory);if(p.monthlyIncome)setMonthlyIncome(p.monthlyIncome);if(p.targetAlloc)setTargetAlloc(p.targetAlloc);if(p.darkMode!==undefined)setDarkMode(p.darkMode);if(p.transactions)setTransactions(p.transactions);if(p._isDemo)setIsDemo(true);}}else{if(isDemo){sessionStorage.removeItem('patra-demo');window.history.replaceState(null,'','/app');const d=DEMO_DATA;setPea(d.pea);setCrypto(d.crypto);setCto(d.cto);setLivrets(d.livrets);setPeaCash(d.peaCash);setCtoCash(d.ctoCash);setCryptoCash(d.cryptoCash||0);setStablecoins(d.stablecoins||[]);setVersements(d.versements);setSnapshots(d.snapshots);setDivHistory(d.divHistory);setMonthlyIncome(d.monthlyIncome);if(d.targetAlloc)setTargetAlloc(d.targetAlloc);setTransactions(d.transactions);setIsDemo(true);}else{setShowOnboarding(true);setOnboardingStep(0);}}}catch(e){}setLoaded(true);},[]);
  ...
  // Backup / Restore
  const clearDemo=()=>{setPea([]);setCrypto([]);setCto([]);setLivrets([]);setPeaCash(0);setCtoCash(0);setCryptoCash(0);setStablecoins([]);setVersements({pea:0,crypto:0,cto:0});setSnapshots([]);setDivHistory([]);setMonthlyIncome(0);setTargetAlloc({});setTransactions([]);setIsDemo(false);localStorage.removeItem("patrimoine-v6");setShowOnboarding(true);setOnboardingStep(0);};
  ...
  // ── Load demo data ──
  const loadDemo=()=>{const d=DEMO_DATA;setPea(d.pea);setCrypto(d.crypto);setCto(d.cto);setLivrets(d.livrets);setPeaCash(d.peaCash);setCtoCash(d.ctoCash);setCryptoCash(d.cryptoCash||0);setStablecoins(d.stablecoins||[]);setVersements(d.versements);setSnapshots(d.snapshots);setDivHistory(d.divHistory);setMonthlyIncome(d.monthlyIncome);if(d.targetAlloc)setTargetAlloc(d.targetAlloc);setTransactions(d.transactions);setIsDemo(true);};
