...
function MetricCard({label,value,sub,icon:Icon,trend,color}){
  return(<div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"20px 24px",display:"flex",flexDirection:"column",gap:6}}>
    ...
    <span style={{fontSize:value.length>12?15:value.length>9?18:value.length>6?22:26,fontWeight:700,color:color||C.text,fontFamily:"'JetBrains Mono',monospace",letterSpacing:-.5,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{value}</span>
...
export default function PatrimoineTracker(){
  ...
  // Backup / Restore
  const clearDemo=()=>{setIsDemo(false);const raw=localStorage.getItem("patrimoine-v6");if(raw){try{const p=JSON.parse(raw);setPea(p.pea||[]);setCrypto(p.crypto||[]);setCto(p.cto||[]);setLivrets(p.livrets||[]);setPeaCash(p.peaCash||0);setCtoCash(p.ctoCash||0);setCryptoCash(p.cryptoCash||0);setStablecoins(p.stablecoins||[]);setVersements(p.versements||{pea:0,crypto:0,cto:0});setSnapshots(p.snapshots||[]);setDivHistory(p.divHistory||[]);setMonthlyIncome(p.monthlyIncome||0);setTargetAlloc(p.targetAlloc||{});setTransactions(p.transactions||[]);}catch(e){}}else{setPea([]);setCrypto([]);setCto([]);setLivrets([]);setPeaCash(0);setCtoCash(0);setCryptoCash(0);setStablecoins([]);setVersements({pea:0,crypto:0,cto:0});setSnapshots([]);setDivHistory([]);setMonthlyIncome(0);setTargetAlloc({});setTransactions([]);setShowOnboarding(true);setOnboardingStep(0);}};
  ...
  return(<div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:"'Outfit',-apple-system,sans-serif",overflowX:"hidden"}}>
    ...
    <Modal show={showTxModal} onClose={()=>setShowTxModal(false)} title={t.logTx}>
      ...
      <SelectField label={t.name} value={txForm.holdingId} onChange={v=>{const arr=txForm.account==="pea"?pea:txForm.account==="cto"?cto:crypto;const h=arr.find(x=>x.id===v);setTxForm(p=>({...p,holdingId:v,name:"",price:h?.currentPrice?String(h.currentPrice):""}));}} options={[{value:"new",label:"— Nouvelle ligne —"},...(txForm.account==="pea"?pea:txForm.account==="cto"?cto:crypto).map(h=>({value:h.id,label:h.name}))]}/>
